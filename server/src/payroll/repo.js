...
/**
 * ของค้างของหน้าค่าตอบแทนพนักงาน — ตัวเลขสำหรับกระดิ่งแจ้งเตือน (ดู notify/alerts.js)
 *
 * ทั้งสามตัวคือ "เงินที่ควรถึงมือพนักงานแล้วแต่ยังไม่ถึง" เรียงตามลำดับของสายพาน:
 *   unreleased = ปิดเคสแล้วแต่ยังไม่มีใครกดปล่อย       (ยังไม่เข้ากองรอจ่าย)
 *   unbatched  = ปล่อยแล้วแต่ยังไม่มีรอบไหนกวาดไป       (อยู่ในกองรอจ่าย)
 *   draft_runs = รอบร่างที่เปิดค้างไว้ยังไม่กดบันทึกจ่าย (เงินติดอยู่ในรอบ)
 *
 * เส้นนี้เป็น manager-only ตามหน้าที่มันชี้ไป — notify/alerts.js ตัดกลุ่มนี้ออกจาก payload
 * ของคนที่ไม่ใช่ผู้จัดการ ไม่ใช่ส่งไปแล้วให้หน้าเว็บซ่อน
 */
export async function alertCounts(periodTo) {
  const [unreleased, pendingRows, runs] = await Promise.all([
    unreleasedCases(periodTo),
    /* จำนวน "คน" ที่มีเงินรออยู่ ไม่ใช่จำนวนก้อนเงิน — คนคือหน่วยที่ผู้จัดการตัดสินใจจ่าย
       (เป็นตัวเลขเดียวกับที่แถบ "ปล่อยค่าจ้างแล้วรอเข้ารอบ" บนหน้ารอบจ่ายแสดงอยู่) */
    preview(periodTo),
    sql.one(`SELECT COUNT(*) AS n FROM payroll_runs WHERE status = 'draft'`),
  ]);

  return {
    unreleased,
    unbatched: pendingRows.length,
    draft_runs: Number(runs?.n ?? 0),
  };
}
