...
```bash
npm install
npm run seed     # ใส่ข้อมูลตัวอย่าง 6 คน (ข้ามให้เองถ้ามีข้อมูลอยู่แล้ว)
npm run dev      # รัน API (:4000) + หน้าเว็บ (:5173) พร้อมกัน
```

เปิด http://localhost:5173

ฐานข้อมูลเป็นไฟล์เดียวที่ `server/data/kin.db` (ใช้ `node:sqlite` ที่มากับ Node 22+ จึงไม่ต้องติดตั้ง driver) — ลบไฟล์นี้ทิ้งเพื่อเริ่มใหม่
