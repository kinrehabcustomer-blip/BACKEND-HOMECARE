...
import { VISIT_STATE_LABELS, formatDate, timeText, durationText, distanceText } from '../labels.js';
...
export default function AttendancePage() {
  ...
  const month = /^\d{4}-(0[1-9]|1[0-2])$/.test(get('month')) ? get('month') : thisMonth();
      ...
      {!rows ? (
        ...
        <div className="table-wrap">
          <table className="table table-cards table-2line att-review">
            {/* คอลัมน์ปุ่มจัดการกว้างพอให้ "บันทึกเวลาจริง" กับ "ยกเลิกกะ" อยู่บรรทัดเดียวกัน
                แคบกว่านี้ปุ่มที่สองตกลงไปบรรทัดล่าง กลายเป็นสองคำสั่งวางซ้อนกันแนวตั้ง
                ซึ่งอ่านเหมือนขั้นตอนที่ต้องทำต่อกัน ทั้งที่เป็นทางเลือกคนละทาง
                ยืมจากทุกคอลัมน์เท่าที่ยืมได้ — "ปัญหา" ยืมได้น้อยสุดเพราะป้ายยาวสุด
                ("เช็คอินนอกวันนัด" / "มาสาย 1 ชม. 30 น.") ต้องไม่หักกลางคำ */}
            <colgroup>
              <col style={{ width: '15%' }} />
              <col style={{ width: '21%' }} />
              <col style={{ width: '16%' }} />
