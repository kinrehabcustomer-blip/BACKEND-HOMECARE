...
/**
 * เปิดปฏิทินของช่อง <input type="date"> เมื่อกดที่ไหนก็ได้ในช่อง
 *
 * ปริยายของเบราว์เซอร์คือปฏิทินเปิดเฉพาะตอนกดโดนไอคอนเล็กๆ ท้ายช่อง (กว้างราว 14px)
 * ส่วนที่เหลือของช่องกดแล้วได้แค่เคอร์เซอร์กระพริบ ซึ่งคนส่วนใหญ่อ่านว่า "กดไม่ได้"
 * แล้วก็ไปพิมพ์เอง — ซึ่งบนเครื่องที่ตั้งค่าเป็น mm/dd/yyyy คือทางลัดไปสู่การกรอกวันสลับเดือน
 *
 * showPicker ต้องการ user activation ซึ่งการกดถือว่าผ่านอยู่แล้ว · ครอบ try ไว้เพราะ
 * บางเบราว์เซอร์/บางบริบท (iframe ที่ไม่มีสิทธิ์) โยน error แทนที่จะเงียบ — ปล่อยผ่านได้
 * เพราะอย่างแย่ที่สุดก็แค่กลับไปเป็นพฤติกรรมเดิมของเบราว์เซอร์
 */
export function openDatePicker(e) {
  try {
    e.currentTarget.showPicker?.();
  } catch {
    /* เบราว์เซอร์ไม่ให้เปิดเอง — ไอคอนของมันยังกดได้ตามปกติ */
  }
}
...
export const CASE_EVENT_LABELS = {
  ...
  pay_released: 'ปล่อยค่าจ้าง',
  pay_shares: 'ส่วนแบ่งค่าจ้าง',
};
 ...
 */
/**
 * วันเวลาที่บันทึกไว้ในฐานข้อมูล -> ข้อความอ่านออก (เช่น '19 ส.ค. 2569 16:57')
 *
 * ค่าที่ "ไม่มีโซนเวลาต่อท้าย" คือเวลาไทยอยู่แล้ว — ทุกคอลัมน์เวลาแบบข้อความในระบบนี้
 * ถูกเขียนด้วย to_char(now() AT TIME ZONE 'Asia/Bangkok', ...) (ดู schema.sql)
 * จึงต้องอ่านตามตัวอักษรตรงๆ ห้ามแปลงโซนเวลาซ้ำ
 *
 * ของเดิมเติม 'Z' ให้ค่าพวกนี้ (สมัยที่ทั้งระบบยังเก็บเป็น UTC) แล้วให้เบราว์เซอร์แปลงเป็นเวลาไทยอีกที
 * = บวกเกินไป 7 ชั่วโมงทุกครั้ง · ประวัติการทำรายการที่เกิดตอน 16:57 จึงขึ้นว่า 23:57
 * ซึ่งเป็นตัวเลขที่เอาไปเทียบกับอะไรไม่ได้เลย และเป็นบันทึกที่ต้องใช้ตอบว่าใครทำอะไรตอนไหน
 */
export const stampText = (value) => {
  if (!value) return '—';

  if (typeof value === 'string') {
    const naive = /^([0-9]{4}-[0-9]{2}-[0-9]{2})[ T]([0-9]{2}:[0-9]{2})/.exec(value);
    const hasZone = /[Zz]$|[+-][0-9]{2}:?[0-9]{2}$/.test(value.trim());
    if (naive && !hasZone) return `${formatDate(naive[1])} ${naive[2]}`;
  }

  // มีโซนเวลามาด้วย (TIMESTAMPTZ เช่น เวลาเช็คอิน) — ให้เบราว์เซอร์แปลงเป็นเวลาเครื่องตามปกติ
